...
public:
    ...
    ShadedGeometry(filename,
                   "../Ders7/normal_vert_shader.vert",
                   "../Ders7/normal_frag_shader.frag")
    {
        generateBuffers();    
    }
    ~NormalShaderNode(){
    ...
    void draw(SceneNode *camera){
		...
		glEnableVertexAttribArray(AttributeCamera);
...
private:
    GLuint UniformMVP;
    GLuint AttributePosition,AttributeNormal,AttributeCamera;
	GLuint UniformColor;
